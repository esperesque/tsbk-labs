...
//enum {kNumBalls = 5}; // Change as desired, max 16
int kNumBalls = 5;
...
vec3 getRandomBoardPos(){
    float x = -0.5 + randf()*1.0;//0.5f - randf()*0.5f;
    float z = -1.5 + randf()*3.0;//randf()*1.0f;
    return(vec3(x,0,z));
}
...
void updateWorld()
{
	...
	{
		// YOUR CODE HERE
		//ball[i].v = vec3(1.2, 0, 0.5)*0.0000009;
	}
...
//		v := P * 1/mass
		ball[i].v = ball[i].P * 1.0/(ball[i].mass); // P is linear momentum
//		X := X + v*dT
...
void init()
{
	...
	//ball[3].X = vec3(0, 0, 0.2);
	//ball[4].X = vec3(0.5,0,0);
	//ball[5].X = vec3(0.9,0,0);
	ball[0].P = vec3(0, 0, 0);
	ball[1].P = vec3(0, 0, 0);
	ball[2].P = vec3(0, 0, 0);
	ball[3].P = vec3(0, 0, 0);
