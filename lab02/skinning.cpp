...
void DeformCylinder()
{
	...
	mat4 m0t = T(g_bones[0].pos.x, g_bones[0].pos.y, g_bones[0].pos.z);
	mat4 m1t = T(g_bones[1].pos.x, g_bones[1].pos.y, g_bones[1].pos.z);

	// Invert translation matrices
	...
	for (row = 0; row < kMaxRow; row++)
	{
		for (corner = 0; corner < kMaxCorners; corner++)
		{
			...
			vec3 v0 = ScalarMult((m0*g_vertsOrg[row][corner]), 1.0-w);
			vec3 v1 = ScalarMult((m1*g_vertsOrg[row][corner]), w);
			g_vertsRes[row][corner] = v0+v1;

			// VectorAdd(ScalarMult(MultVec3(Mult(mBen0,invmBen0),g_vertsOrg[row][corner]),1.0-weight[row]),
